...
"sentence-transformers/all-MiniLM-L6-v2")
    ...
    JOBS_PER_TELEGRAM_BATCH: int = int(os.getenv("JOBS_PER_TELEGRAM_BATCH",
"8"))
    DAILY_JOB_LIMIT: int = int(os.getenv("DAILY_JOB_LIMIT", "25"))  # per
platform

    # Location filter
    LOCATION_POSTCODE: str = os.getenv("LOCATION_POSTCODE", "WN7 1NX")
    LOCATION_RADIUS_MILES: float = float(os.getenv("LOCATION_RADIUS_MILES",
"5.0"))

    # Salary filter
    MIN_SALARY_PER_HOUR: float = float(os.getenv("MIN_SALARY_PER_HOUR",
"11.0"))
    ...
    SCRAPE_DELAY_SECONDS: float = float(os.getenv("SCRAPE_DELAY_SECONDS",
"1.0"))

    # Timing (UK local times in 24h format HH:MM)
    SCRAPE_TIMES: list[str] = os.getenv("SCRAPE_TIMES",
"08:30,13:45,17:00").split(",")
    SEND_TIMES: list[str] = os.getenv("SEND_TIMES",
"09:00,18:00,21:00").split(",")

    # Monitoring
    SYSTEM_REPORT_INTERVAL_HOURS: int =
int(os.getenv("SYSTEM_REPORT_INTERVAL_HOURS", "5"))
